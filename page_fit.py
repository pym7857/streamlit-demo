...
def page_fit():
    ...
    st.sidebar.markdown("**시각화 모드 설정:**")
    plot_mode = st.sidebar.radio("Options", ('Dark Mode', 'Light Mode'))   

    st.sidebar.text("")
    st.sidebar.text("")

    st.markdown("<h1 style='text-align: center;'> 내 데이터 분포 확인하기 ⭐️ </h1>", 
    ...
    def plot(df, data_stat):
        ...
        if data_stat:
            ...
            quantiles = df.quantile(q) # quantile = 분위수 
            ...
            ax.vlines(np.std(df), ymin=0, ymax=hist_dist.cdf(np.std(df)),
                      color=std_color, linestyle='--', linewidth=2,
                      label=f'Std {round(np.std(df),2)}')       
    ...
    if input:
        ...
        data_plot = st.checkbox('Plot my data (histogram)')
