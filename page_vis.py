...
def page_vis(): 
    ...
    with st.container():
        st.title("파이썬 데이터 시각화👀")
        st.header("유명한 시각화 라이브러리들")
        st.write("""5개 라이브러리에 대해 코드를 살펴봅시다""")


    # User choose type
    chart_type = st.selectbox("차트유형을 선택해주세요", plot_types)

    with st.container():
        st.subheader(f"현재 선택된 유형:  {chart_type}")
        st.write("")
